...
- Dynamic config loading is covered by a regression test and reported as a scope limitation instead of a clean static result.
- Non-npm/pnpm package managers are covered by a regression test so temporary Vite 8 comparison does not silently fall through to npm.
